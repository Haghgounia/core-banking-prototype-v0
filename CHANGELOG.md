## 0.3.74 — CAL2: بازنگری Grid روزهای تقویم کاری

- نمایش `CAL2.BUSINESS_CALENDAR_DAY` از حالت شناسه‌محور به نمای کسب‌وکاری ارتقا یافت.
- ستون «تاریخ شمسی» و «روز هفته» با Join به `CANONICAL_DAY/CALENDAR_DATE/WEEKDAY` اضافه شد.
- شناسه تقویم و منبع در Grid به عنوان فارسی `BUSINESS_CALENDAR.NAME_FA` و `SOURCE_AUTHORITY.NAME_FA` تبدیل می‌شوند.
- مقادیر فنی وضعیت و دلیل با عنوان فارسی نمایش داده می‌شوند، بدون تغییر Code ذخیره‌شده در Oracle.
- جستجو و Sort صفحه برای تاریخ شمسی، تقویم، روز هفته، وضعیت، دلیل و منبع غنی شد.
