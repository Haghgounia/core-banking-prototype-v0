...
- منوی «اطلاعات پایه عمومی» سه گزینه مرتبط با واژه‌ها را در گروه مستقل «واژگان و اصطلاحات» نگه می‌دارد: واژه‌نامه رومن‌نویسی، واژه‌نامه پیشوند/پسوند و ابزار تبدیل نام فارسی به انگلیسی.
- سازگاری با Backend قدیمی حفظ شد؛ اگر دو واژه‌نامه هنوز با Category قدیمی `GENERAL` برگردند، Frontend آن‌ها را همچنان در گروه واژگان نمایش می‌دهد.
- شمارنده دامنه عمومی اکنون `VOCABULARY` و `EDU_REFERENCE` را نیز محاسبه می‌کند.
- فیلترهای تخصصی هر شش فرم Canonical در Schema `EDU` تثبیت شدند و فرم رشته‌ها/گرایش‌ها علاوه بر نظام آموزشی، نوع گره، قابل انتخاب و وضعیت، فیلتر «رشته/گروه والد» نیز دارد.
- Guard جدید `verify-edu-reference-ui.mjs` قرارداد منو، شش مجموعه فیلتر، ارسال `filter.*` و اعمال پارامتری آن در Oracle Repository را هنگام Build کنترل می‌کند.
- ناسازگاری Release Metadata نسخه 0.3.82 رفع شد: `VERSION`، Maven POM، Frontend package و System Specification همگی روی 0.3.83 همگام هستند و Install Guide متناظر وجود دارد.
- Source Package دیگر `app/BUILD-VERSION` را حمل نمی‌کند؛ بنابراین Extract روی نصب قبلی نمی‌تواند JAR قدیمی را به‌اشتباه Build نسخه جدید معرفی کند.
- تغییر DDL یا Migration پایگاه داده لازم نیست.
...
- افزودن `CIF.REF_RELIGIOUS_DENOMINATION` برای مذهب/شاخه دینی با FK به دین و سلسله‌مراتب اختیاری.
- افزودن دو فرم Metadata-driven در «اطلاعات پایه مشتری / Party → هویت و Party».
- Seed اولیه فارسی/انگلیسی و Migration idempotent برای Oracle.
- بدون اتصال اجباری به PERSON در این فاز؛ اتصال به پروفایل شخص حقیقی در گام بعدی انجام می‌شود.

## 0.3.78 — FIX86: پیش‌فرض سال هجری شمسی جاری در فرم‌های تاریخ‌محور CAL

- فرم‌های تاریخ‌محور CAL هنگام ورود به‌صورت پیش‌فرض فقط داده‌های سال هجری شمسی جاری را نمایش می‌دهند؛ داده‌های سایر سال‌ها در دیتابیس بدون تغییر باقی می‌مانند.
- این قاعده برای `CALENDAR_DAY`, `CALENDAR_DATE`, `BUSINESS_CALENDAR_DAY`, `CALENDAR_EXCEPTION`, `OCCASION_OCCURRENCE`, `CALENDAR_DAY_OCCASION` و `HIJRI_DATE_OVERRIDE` اعمال شد.
- سال جاری، حداقل سال و حداکثر سال مستقیماً از Dataset شمسی `CAL.CALENDAR_DATE` استخراج می‌شود؛ در وضعیت فعلی سال جاری ۱۴۰۵ است.
- کاربر می‌تواند سال را تغییر دهد، با «سال جاری» به مقدار پیش‌فرض برگردد یا با «همه سال‌ها» کل بازه را مشاهده کند.
- فیلتر سال در Backend و روی `YEAR_NO` تقویم `SOLAR_HIJRI_IR` اعمال می‌شود تا صفحه‌بندی و شمارش روی کل Dataset چندصدساله انجام نشود.
- جداول Master/Reference مانند سیستم تقویم، ماه، روز هفته، تعریف مناسبت و قواعد مناسبت عمداً فیلتر سال ندارند.
- Guard جدید `verify-calendar-current-year-default.mjs` قرارداد UI/API/SQL این رفتار را کنترل می‌کند.
