...
const checks=[
 ...
 [migration.includes('UX_DEP_OPEN_REQ_IDEMPOTENCY')&&migration.includes('UX_DEP_OPEN_BATCH_IDEMPOTENCY')&&migration.includes('UX_DEP_OPEN_BATCH_EXT_ROW_KEY'),'Phase 7 unique indexes are incomplete'],
 [migration.includes('-20701')&&migration.includes('-20702')&&migration.includes('-20703'),'legacy duplicate fail-fast guards are missing'],
 [readiness.includes('APPEND_ONLY')&&readiness.includes('IDEMPOTENCY')&&runtimeRepo.includes('ALL_TRIGGERS')&&runtimeRepo.includes('ALL_INDEXES'),'runtime readiness metadata checks are incomplete'],
 [readiness.includes('PROPAGATION_REQUIRES_NEW')&&readiness.includes('setRollbackOnly')&&runtimeRepo.includes('PHASE7_ROLLBACK_PROBE'),'isolated rollback probe is missing'],
