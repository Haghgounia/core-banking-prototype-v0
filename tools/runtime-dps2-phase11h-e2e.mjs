...
async function resolveAllowedTerm(productVersionId){
  ...
  for(const rule of activeRules){
    ...
    for(const row of allowed.filter(activeNow)){
      try{return {...allowedTermFromRow(row,allowedDescriptor),maturityActionCode}}catch(e){console.log(`PHASE11H_RUNTIME_E2E_SKIP_ALLOWED_TERM=${text(row.ALLOWED_TERM_ID||row[allowedDescriptor.primaryKeyColumn])} reason=${String(e.message).slice(0,180)}`)}
    }
  }
...
async function qualifyOutstandingPayablePeriod(){
  for(const a of await candidateAccounts()){
    try{
      const beforeDetail=await req('GET',`/api/v1/deposit-accounts/${a.accountId}`);
      const beforeProfit=await req('GET',`/api/v1/deposit-accounts/${a.accountId}/profit`);
      const outstanding=(beforeProfit.periods||[]).filter(x=>text(x.periodStatusCode).toUpperCase()!=='CLOSED'&&Number(x.payableAmount)>Number(x.paidAmount)+0.0001);
      if(!outstanding.length||!(Number(beforeProfit.contract?.accruedAmount)>0))continue;
      const beforeById=new Map(outstanding.map(x=>[Number(x.profitPeriodId),Number(x.payableAmount)-Number(x.paidAmount)]));
      const ledgerBefore=Number(beforeDetail.balance.ledgerBalance);
      const key='11h-reopen-post-'+uid();
      const posted=await req('POST',`/api/v1/deposit-accounts/${a.accountId}/profit/postings`,{postingDate:today()},'deposit.operator',key);
      if(!(posted.entityId>0))throw new Error('REOPENED_PERIOD_POSTING_ID_MISSING');
      const afterProfit=await req('GET',`/api/v1/deposit-accounts/${a.accountId}/profit`);
      const afterDetail=await req('GET',`/api/v1/deposit-accounts/${a.accountId}`);
      const payment=(afterProfit.payments||[]).find(x=>x.paymentStatusCode==='PAID'&&x.postingReference===`P11H-PROFIT-${posted.entityId}`);
      if(!payment)throw new Error('REOPENED_PERIOD_PAYMENT_TRACE_MISSING');
      const expected=beforeById.get(Number(payment.profitPeriodId));if(!(expected>0))throw new Error('REOPENED_PERIOD_SELECTION_MISMATCH');
      const period=(afterProfit.periods||[]).find(x=>Number(x.profitPeriodId)===Number(payment.profitPeriodId));
      if(!period||Math.abs(Number(period.payableAmount)-Number(period.paidAmount))>0.0001||text(period.periodStatusCode).toUpperCase()!=='PAID')throw new Error('REOPENED_PERIOD_NOT_SETTLED');
      const ledgerDelta=Number(afterDetail.balance.ledgerBalance)-ledgerBefore;if(Math.abs(ledgerDelta-expected)>0.0001)throw new Error(`REOPENED_PERIOD_LEDGER_DELTA_MISMATCH:${ledgerDelta} expected=${expected}`);
      const replay=await req('POST',`/api/v1/deposit-accounts/${a.accountId}/profit/postings`,{postingDate:today()},'deposit.operator',key);if(replay.idempotentReplay!==true)throw new Error('REOPENED_PERIOD_POSTING_REPLAY_MISSING');
      console.log(`PHASE11H_RUNTIME_REOPENED_PERIOD_ACCOUNT=${a.accountId}`);
      console.log(`PHASE11H_RUNTIME_REOPENED_PERIOD=${payment.profitPeriodId}`);
      console.log(`PHASE11H_RUNTIME_REOPENED_PERIOD_POSTING=${posted.entityId}`);
      console.log(`PHASE11H_RUNTIME_REOPENED_PERIOD_PAYMENT=${payment.profitPaymentId}`);
      console.log('PHASE11H_RUNTIME_REOPENED_PERIOD_PASS');
      return true;
    }catch(e){
      console.log(`PHASE11H_RUNTIME_REOPENED_PERIOD_SKIP_ACCOUNT=${a.accountId} reason=${String(e.message).slice(0,180)}`);
    }
  }
  console.log('PHASE11H_RUNTIME_REOPENED_PERIOD_NONE');
  return false;
}
async function findProfitCandidate(){
  ...
  for(const a of await candidateAccounts()){
    try{
      ...
      console.log(`PHASE11H_RUNTIME_E2E_SKIP_ACCOUNT=${a.accountId} reason=${String(e.message).slice(0,180)}`);
    }
  }
  if(profitStorageFailure)throw new Error('PHASE11H_PROFIT_STORAGE_NOT_READY_OR_SCHEMA_MISMATCH; run tools/apply-dps2-phase11g.cmd and require PHASE11H_DB_BASELINE_PASS before Runtime E2E');
  return null;
}
  ...
  for(const product of termProducts){
    ...
    for(const version of versions.filter(openingEligibleVersion)){
      ...
      try{
        ...
        console.log(`PHASE11H_RUNTIME_E2E_PDL_TEMPLATE_PRODUCT_VERSION=${productVersionId} family=${text(product.PRODUCT_FAMILY_CODE)}`);
        ...
        console.log(`PHASE11H_RUNTIME_E2E_SKIP_PDL_PRODUCT_VERSION=${productVersionId} reason=${String(e.message).slice(0,220)}`);
      }
    }
  }
  return null;
...
async function bootstrapTemplate(){
  ...
  for(const template of templates){
    ...
    try{
      ...
      catch(e){profitGap=true;console.log(`PHASE11H_RUNTIME_E2E_SKIP_TEMPLATE_PROFIT_PRODUCT_VERSION=${productVersionId} reason=${String(e.message).slice(0,220)}`)}
    }catch(e){console.log(`PHASE11H_RUNTIME_E2E_SKIP_TEMPLATE_PRODUCT_VERSION=${productVersionId} reason=${String(e.message).slice(0,220)}`)}
  }
...
async function bootstrapProfitAccount(){
  ...
  const run=uid();const amount=1000;const evidence=`P11H-${run}`;const actor='phase11h.qa';const start=addDays(today(),-1);const maturity=addTerm(start,allowedTerm.termValue,allowedTerm.termUnitCode);
  console.log(`PHASE11H_RUNTIME_E2E_ALLOWED_TERM=${allowedTerm.allowedTermId} code=${allowedTerm.termCode} value=${allowedTerm.termValue} unit=${allowedTerm.termUnitCode}`);
  console.log(`PHASE11H_RUNTIME_E2E_PRICING rule=${profitConfig.pricingRuleId} component=${profitConfig.pricingComponentId} tier=${profitConfig.rateTierId||''} paymentRule=${profitConfig.profitPaymentRuleId} rate=${profitConfig.rateValue}`);
  const aggregate={
    DEPOSIT_OPENING_REQUEST:{REQUEST_NO:`P11H-${run}`.slice(0,40),IDEMPOTENCY_KEY:`P11H-${run}`.slice(0,80),PRODUCT_VERSION_ID:Number(template.productVersionId),REQUEST_TYPE_CODE:'CUSTOMER_REQUEST',OWNERSHIP_TYPE_CODE:'INDIVIDUAL',CURRENCY_CODE:'IRR',OPENING_CHANNEL_CODE:'BRANCH',ORG_UNIT_CODE:'001',REQUESTED_OPENING_DATE:start,OPENING_AMOUNT:amount,SOURCE_OF_FUNDS_CODE:'SAVINGS',PURPOSE_CODE:'INVESTMENT',CUSTOMER_RISK_LEVEL_CODE:'LOW',RISK_ASSESSMENT_REFERENCE:`${evidence}-RISK`.slice(0,120),EXPECTED_ACTIVITY_REFERENCE:`${evidence}-ACTIVITY`.slice(0,120),ACTIVATION_STATUS_CODE:'NOT_CREATED',REQUEST_STATUS_CODE:'APPROVED'},
    ...
    DEPOSIT_OPENING_OBLIGATION:[{OPENING_OBLIGATION_ID:1,OBLIGATION_TYPE_CODE:'INITIAL_BALANCE',SOURCE_SYSTEM_CODE:'PHASE11H',SOURCE_REFERENCE:`${evidence}-INITIAL`.slice(0,120),DESCRIPTION:'Phase 11H funded term runtime account',GROSS_AMOUNT:amount,WAIVED_AMOUNT:0,FINAL_AMOUNT:amount,CURRENCY_CODE:'IRR',MANDATORY_FOR_ACTIVATION_FLAG:1,SETTLEMENT_STATUS_CODE:'PENDING'}],
    ...
    DEPOSIT_OPENING_CHECK:createGateChecks.map(([code,type,phase],i)=>({CHECK_CODE:code,CHECK_TYPE_CODE:type,ATTEMPT_NO:1,CHECK_PHASE_CODE:phase,BLOCKING_SCOPE_CODE:'ACCOUNT_CREATION',REQUIRED_FLAG:1,RECHECK_REQUIRED_FLAG:0,RESULT_STATUS_CODE:'PASS',RESULT_REFERENCE:`${evidence}-CG-${String(i+1).padStart(2,'0')}`.slice(0,120),SOURCE_EVALUATION_REFERENCE:'PHASE11H_RUNTIME_BOOTSTRAP'})),
    DEPOSIT_OPENING_TERMS_ACCEPTANCE:{TERMS_VERSION_CODE:'PHASE11H-2026.09',ACCEPTED_BY_PARTY_ID:Number(template.primaryPartyId),ACCEPTANCE_SOURCE_CODE:'API',CHANNEL_CODE:'BRANCH',ACCEPTANCE_STATUS_CODE:'ACCEPTED',EVIDENCE_REFERENCE:`${evidence}-TERMS`.slice(0,120)},
    DEPOSIT_OPENING_DECISION:{DECISION_CODE:'APPROVE',DECISION_REASON_CODE:'ALL_CHECKS_PASSED',DECISION_NOTE:'Phase 11H runtime bootstrap',DECIDED_BY:actor}
  };
  ...
  const readinessEvidence=['CBI_SIAH_REGISTRATION','FINAL_COMPLIANCE_RECHECK','RESTRICTIONS_READY'].map(code=>({CHECK_CODE:code,RESULT_STATUS_CODE:'PASS',RESULT_REFERENCE:`${evidence}-${code}`.slice(0,120),VALID_UNTIL:plusHours(2),SOURCE_EVALUATION_REFERENCE:'PHASE11H_RUNTIME_BOOTSTRAP'}));
  ...
  console.log(`PHASE11H_RUNTIME_E2E_BOOTSTRAP_ACCOUNT=${account.accountId}`);
  console.log(`PHASE11H_RUNTIME_E2E_PROFIT_CONTRACT=${profit.contract.profitContractId} rate=${profit.contract.annualRate} dayCount=${profit.contract.dayCountBasisCode}`);
  return {account:{...template,accountId:account.accountId},detail,profit};
}
try{
  await qualifyOutstandingPayablePeriod();
  let selected=await findProfitCandidate();if(!selected)selected=await bootstrapProfitAccount();
  const {account}=selected;const id=account.accountId;console.log(`PHASE11H_RUNTIME_E2E_ACCOUNT=${id}`);
  const beforeProfit=await req('GET',`/api/v1/deposit-accounts/${id}/profit`);
  if(!beforeProfit.profile||!(beforeProfit.profile.accountProfitProfileId>0))throw new Error('CANONICAL_PROFILE_MISSING');
  ...
  const accrualKey='11h-accrue-'+uid();const accrued=await req('POST',`/api/v1/deposit-accounts/${id}/profit/accruals`,{throughDate:through},'deposit.operator',accrualKey);
  if(!(accrued.entityId>0))throw new Error('PROFIT_ACCRUAL_ID_MISSING');console.log(`PHASE11H_RUNTIME_E2E_ACCRUAL=${accrued.entityId}`);
  const afterAccrual=await req('GET',`/api/v1/deposit-accounts/${id}/profit`);
  const accrual=afterAccrual.accruals.find(x=>Number(x.profitAccrualId)===Number(accrued.entityId));
  if(!accrual||!(accrual.profitPeriodId>0))throw new Error('PROFIT_PERIOD_TRACE_MISSING');
  const detail=afterAccrual.accrualDetails.find(x=>Number(x.profitAccrualId)===Number(accrued.entityId)&&x.componentCode==='BASE_PROFIT');
  if(!detail)throw new Error('ACCRUAL_DETAIL_TRACE_MISSING');
  const period=afterAccrual.periods.find(x=>Number(x.profitPeriodId)===Number(accrual.profitPeriodId));if(!period)throw new Error('PROFIT_PERIOD_MISSING');
  console.log(`PHASE11H_RUNTIME_E2E_PERIOD=${period.profitPeriodId}`);console.log(`PHASE11H_RUNTIME_E2E_DETAIL=${detail.profitAccrualDetailId}`);
  ...
  const adjKey='11h-adj-'+uid();const adj=await req('POST',`/api/v1/deposit-accounts/${id}/profit/adjustments`,{originalAccrualId:accrued.entityId,originalPaymentId:null,adjustmentTypeCode:'INCREASE',adjustmentAmount:1,reasonCode:'RUNTIME_QUALIFICATION',approverUserId:'deposit.approver',orgUnitCode:'HQ'},'deposit.operator',adjKey);
  if(!(adj.entityId>0))throw new Error('ADJUSTMENT_ID_MISSING');console.log(`PHASE11H_RUNTIME_E2E_ADJUSTMENT=${adj.entityId}`);
  let adjView=await req('GET',`/api/v1/deposit-accounts/${id}/profit`);let a=adjView.adjustments.find(x=>Number(x.profitAdjustmentId)===Number(adj.entityId));if(!a||a.adjustmentStatusCode!=='DRAFT'||a.approvalStatusCode!=='PENDING_APPROVAL')throw new Error('ADJUSTMENT_APPROVAL_MISSING');
  await req('POST',`/api/v1/deposit-accounts/${id}/profit/adjustments/${adj.entityId}/approve`,{},'deposit.approver','11h-adj-approve-'+uid());
  await req('POST',`/api/v1/deposit-accounts/${id}/profit/adjustments/${adj.entityId}/post`,{},'deposit.operator','11h-adj-post-'+uid());
  adjView=await req('GET',`/api/v1/deposit-accounts/${id}/profit`);a=adjView.adjustments.find(x=>Number(x.profitAdjustmentId)===Number(adj.entityId));if(!a||a.adjustmentStatusCode!=='POSTED'||a.approvalStatusCode!=='APPROVED')throw new Error('ADJUSTMENT_NOT_POSTED');

  const payableBefore=Number(adjView.contract.accruedAmount);if(!(payableBefore>0))throw new Error('NO_PAYABLE_PROFIT_AFTER_ADJUSTMENT');
  const ledgerBefore=Number((await req('GET',`/api/v1/deposit-accounts/${id}`)).balance.ledgerBalance);
  const postKey='11h-post-'+uid();const posted=await req('POST',`/api/v1/deposit-accounts/${id}/profit/postings`,{postingDate:through},'deposit.operator',postKey);if(!(posted.entityId>0))throw new Error('PROFIT_POSTING_ID_MISSING');console.log(`PHASE11H_RUNTIME_E2E_POSTING=${posted.entityId}`);
  const finalProfit=await req('GET',`/api/v1/deposit-accounts/${id}/profit`);const finalDetail=await req('GET',`/api/v1/deposit-accounts/${id}`);
  const payment=finalProfit.payments.find(x=>x.paymentStatusCode==='PAID'&&x.postingReference===`P11H-PROFIT-${posted.entityId}`);if(!payment)throw new Error('PROFIT_PAYMENT_TRACE_MISSING');console.log(`PHASE11H_RUNTIME_E2E_PAYMENT=${payment.profitPaymentId}`);
  const finalPeriod=finalProfit.periods.find(x=>Number(x.profitPeriodId)===Number(payment.profitPeriodId));if(!finalPeriod||Number(finalPeriod.paidAmount)<=0)throw new Error('PROFIT_PERIOD_PAYMENT_NOT_APPLIED');
  const ledgerDelta=Number(finalDetail.balance.ledgerBalance)-ledgerBefore;if(Math.abs(ledgerDelta-payableBefore)>0.0001)throw new Error(`PROFIT_LEDGER_DELTA_MISMATCH:${ledgerDelta} expected=${payableBefore}`);
  ...
  const pr=finalProfit.postings.find(x=>Number(x.profitPostingId)===Number(posted.entityId));if(!pr||pr.statusCode!=='POSTED'||!(pr.subledgerEntryId>0))throw new Error('PROFIT_POSTING_TRACE_MISSING');
  const sl=(finalDetail.subledgerEntries||[]).find(x=>Number(x.subledgerEntryId)===Number(pr.subledgerEntryId));if(!sl||sl.sourceEntityType!=='DEPOSIT_PROFIT_POSTING'||Number(sl.sourceEntityId)!==Number(posted.entityId))throw new Error('PROFIT_SUBLEDGER_TRACE_MISMATCH');
  ...
  console.log('PHASE11H_RUNTIME_E2E_PASS');
}catch(e){console.error('PHASE11H_RUNTIME_E2E_FAIL');console.error(e);process.exit(1)}
