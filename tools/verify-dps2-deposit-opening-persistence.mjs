...
const checks = [
  [['0.3.98','0.3.99'].includes(version), `VERSION must preserve Phase 2 or later source, got ${version}`],
  ...
  [repository.includes('Initial aggregate persistence') && service.includes('insertInitialStatusHistory'), 'system-managed initial status history is missing'],
  ...
  [wizardTs.includes('persistAggregate()') && wizardHtml.includes('ثبت اتمیک Aggregate') && wizardHtml.includes('OPENING_REQUEST_ID='), 'wizard is not connected to atomic persistence'],
