...
// FIX97 - six-step wizard and hierarchical child navigation
must(workspaceHtml.includes('مرحله ۱') || workspaceHtml.includes('step-num">۱'), 'Six-step wizard step 1 missing');
must(workspaceHtml.includes('مرحله ۶') || workspaceHtml.includes('step-num">۶'), 'Six-step wizard step 6 missing');
must(workspace.includes('MODULE_DEFINITIONS'), 'Module-driven wizard definition missing');
must(workspace.includes('toggleModule('), 'Inline PRODUCT_VERSION_MODULE toggle missing');
must(workspace.includes("ORIGINATION_STATUS_CODE: new FormControl('CLOSED'"), 'Origination target semantics missing');
must(workspace.includes("SERVICING_STATUS_CODE: new FormControl('CLOSED'"), 'Servicing target semantics missing');
must(workspaceHtml.includes('value="OPEN"') && workspaceHtml.includes('value="SUSPENDED"') && workspaceHtml.includes('value="CLOSED"'), 'Origination OPEN/SUSPENDED/CLOSED options missing');
must(workspaceHtml.includes('value="ACTIVE"') && workspaceHtml.includes('خدمت‌رسانی'), 'Servicing ACTIVE/SUSPENDED/CLOSED options missing');
must(tableTs.includes('CHILD_TABLES'), 'Hierarchical child-table mapping missing');
for (const child of ['PRODUCT_CHANNEL_OPERATION','PRODUCT_PRICING_COMPONENT','PRODUCT_RATE_TIER','DEPOSIT_PRODUCT_ALLOWED_TERM','DEPOSIT_PRODUCT_CLOSURE_PRECHECK','DEPOSIT_PRODUCT_CLOSURE_SETTLEMENT_RULE','DEPOSIT_PRODUCT_CLOSURE_APPROVAL_RULE','CORRESPONDENT_ACCOUNT_SETTLEMENT_RULE','LOAN_ELIGIBILITY_EXTENSION']) {
  must(tableTs.includes(child), `Hierarchical child navigation missing: ${child}`);
}
must(tableHtml.includes('childLinks(row)') && tableHtml.includes('childFilterValue(row, child)'), 'Child row actions not rendered');
must(workspaceHtml.includes('بازبینی و آمادگی محصول'), 'Wizard review step missing');
// FIX99 - Angular/TypeScript noPropertyAccessFromIndexSignature compile guard
must(workspace.includes("delete payload['APPROVED_AT']"), 'APPROVED_AT must use bracket access on Record index signature');
must(workspace.includes("delete payload['APPROVED_BY']"), 'APPROVED_BY must use bracket access on Record index signature');
must(!workspace.includes('delete payload.APPROVED_AT'), 'Unsafe dot access remains for APPROVED_AT');
must(!workspace.includes('delete payload.APPROVED_BY'), 'Unsafe dot access remains for APPROVED_BY');
// FIX100 - overlay upgrades must not retain the obsolete transitional governance service.
must(!exists('backend/src/main/java/com/behsazan/corebanking/productbuilder/application/ProductGovernanceService.java'), 'Obsolete ProductGovernanceService.java remains in canonical source');
