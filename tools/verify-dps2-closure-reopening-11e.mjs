...
check('positive ledger settles through 11D posting primitive',text.svc.includes('balanceService.post(accountId'));
check('closure posting source is ACCOUNT_CLOSURE',text.svc.includes('"ACCOUNT_CLOSURE",closureId'));
...
check('closure status history emitted',text.svc.includes('"CLOSURE:"+closureId'));
check('reopening status history emitted',text.svc.includes('"REOPENING:"+reopeningId'));
