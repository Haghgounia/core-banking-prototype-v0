...
## چهار سپرده / Deposit Account Opening — Phase 2 (0.3.98)

در این نسخه Wizard افتتاح موردی از حالت Payload-only خارج شده و Endpoint اتمیک `POST /api/v1/deposit-opening/requests` به Backend اضافه شده است. Aggregate افتتاح شامل درخواست، Partyها، Rule/Term، تأمین وجه، کنترل‌ها، پذیرش شروط، تصمیم و Status History در یک Transaction روی Schema `DPS2` ثبت می‌شود؛ در صورت خطا کل عملیات Rollback خواهد شد. کنترل Idempotency نیز از ایجاد دوباره همان درخواست جلوگیری می‌کند. ایجاد `DEPOSIT_ACCOUNT` هنوز عمداً در مرحله بعدی چرخه عمر قرار دارد.

## تغییرات نسخه 0.3.96

## چهار سپرده / Deposit Account Opening (0.3.97)

منوی «چهار سپرده» دامنه افتتاح چهار خانواده حساب قرض‌الحسنه پس‌انداز، جاری، کوتاه‌مدت و بلندمدت را روی Schema قابل تنظیم `DPS2` ارائه می‌کند. این فاز شامل 59 فرم اطلاعات پایه، 25 فرم عملیاتی عمومی و Wizard هفت‌مرحله‌ای افتتاح موردی است. گواهی سپرده خارج از این bounded context است و جداول Audit/Change در Generic CRUD عرضه نمی‌شوند.


- FIX104: خطای `ORA-06502` در Source-to-Oracle Reconciliation برای متن‌های فارسی چندبایتی رفع شد.
- بافرهای موقت PL/SQL به‌جای `VARCHAR2(250/500/1000)` به `%TYPE` ستون‌های واقعی Oracle متصل شدند؛ بنابراین طول CHAR ستون‌ها و متن فارسی به‌درستی رعایت می‌شود.
- این Fix داده‌های کارمزد را تغییر نمی‌دهد. پس از اجرای موفق `01-import` و `02-verify`، فقط `04-reconcile-cbi-rial-fee-1405-provisional.sql` اصلاح‌شده را اجرا کنید و در صورت `mismatches=0` سپس `COMMIT` نمایید.

## تغییرات نسخه 0.3.95

- FIX103: اجرای مستقل Source-to-Oracle Reconciliation کارمزد Transaction-neutral شد و دیگر Transaction کاربر را `COMMIT/ROLLBACK` نمی‌کند.
- Installer همچنان حالت سخت‌گیرانه دارد و قبل از Commit، Reconciliation را با Rollback-on-error اجرا می‌کند.
- اسکریپت `05-diagnose-cbi-rial-fee-1405-state.sql` برای تشخیص وضعیت نصب، Service/PDB و بازگشت احتمالی Import اضافه شد.
- اگر خروجی Reconciliation همه 152 تعرفه را `missing` و Global Countها را صفر نشان دهد، ابتدا Diagnostic اجرا و سپس Installer کامل `00-install...sql` اجرا شود؛ اجرای مستقیم `01`/`02` برای بازیابی توصیه نمی‌شود.

## تغییرات نسخه 0.3.94

- FIX102: خطای Syntax اسکریپت Read-only تطبیق Source-to-Oracle کارمزد اصلاح شد.
- `LIKE 'SRC:%'` و جداکننده‌های `'#'`/`'#TIER'` در PL/SQL اکنون literal معتبر Oracle هستند.
- Generator و Verifier نیز اصلاح شدند تا همین خطا در بازتولید فایل برنگردد.
- دیتابیس نیاز به Import یا Migration مجدد ندارد؛ فقط اسکریپت `04-reconcile-cbi-rial-fee-1405-provisional.sql` اصلاح‌شده را اجرا کنید.

## تغییرات نسخه 0.3.93

- FIX101: برای مصوبه موقت کارمزد خدمات بانکی ریالی ۱۴۰۵، کنترل کامل Source-to-Oracle اضافه شد.
- اسکریپت `database/oracle/fee/cbi-rial-1405-provisional/04-reconcile-cbi-rial-fee-1405-provisional.sql` تمام 152 تعرفه، 180 جزء محاسباتی، 66 Input و 15 Tier را با قرارداد منبع تطبیق می‌دهد.
- اجرای این اسکریپت Read-only است؛ در صورت اختلاف `ORA-20260` برمی‌گرداند و در حالت سالم پیام `CBI Rial Fee 1405 PROVISIONAL source-to-Oracle reconciliation OK.` چاپ می‌شود.
- هیچ DDL یا تغییر داده جدیدی ندارد.

## تغییرات نسخه 0.3.81

- اصلاح Runtime Materializer تقویم دو برای اجرای قطعی `LAST_DAY_IF_INVALID` داخل Oracle SQL.
- بدون تغییر DDL؛ فقط Backend باید Build/Deploy شود.
- پس از Deploy، ابتدا Rule 193 را جداگانه Rebuild کنید و سپس در صورت موفقیت `rebuild-all` را اجرا کنید.

## تغییرات نسخه 0.3.80

- CAL2: سیاست صریح برای روز نامعتبر در قواعد تکرار مناسبت افزوده شد.
- Migration: `database/oracle/cal2/migrations/0.3.80-fix88-last-day-resolution-policy.sql`
- برای «شهادت امام رضا (ع) / ۳۰ صفر»، قاعده ۳۰ صفر حفظ می‌شود و در Variant محاسباتی فعلی فقط هنگام نبود روز ۳۰، رخداد روی آخرین روز صفر ساخته می‌شود.
- پس از Migration، `rebuild-all` را اجرا و Validation رخدادها را تکرار کنید.

## تغییرات نسخه 0.3.79

- فرم «دین» و «مذهب / شاخه دینی» در اطلاعات پایه Party اضافه شد.
- Migration: `database/oracle/cif/migrations/0.3.79-fix87-religion-denomination-reference.sql`

## تغییرات نسخه 0.3.78

- در تمام فرم‌های تاریخ‌محور «تقویم یک»، سال هجری شمسی جاری به‌صورت پیش‌فرض اعمال می‌شود.
- فرم‌های مشمول: روزهای مرجع، نمایش تاریخ در سه تقویم، روزهای تقویم کاری، استثناهای تقویم کاری، رخدادهای مناسبت، مناسبت‌های روز و اصلاحات رسمی تاریخ قمری.
- فیلتر سال قابل تغییر است و گزینه‌های «سال جاری» و «همه سال‌ها» دارد؛ هیچ داده تاریخی حذف یا محدود نمی‌شود.
- سال جاری از خود Dataset `CAL.CALENDAR_DATE` خوانده می‌شود و Filter به‌صورت Server-side اجرا می‌شود.
- این نسخه DDL جدید ندارد.

## تغییرات نسخه 0.3.77

- نمای ماهانه «تقویم یک» در مسیر `/calendar/month-view` اضافه شده و از سه تقویم شمسی، میلادی و قمری پشتیبانی می‌کند.
- فرم‌های پرحجم CAL برای نمایش تاریخ شمسی، روز هفته و عنوان‌های فارسی/کسب‌وکاری بازبینی شده‌اند.
- داده مناسبت‌های CAL از CAL2 همگام شده است؛ برای کنترل پوشش دو مدل از اسکریپت Read-only `CAL_CAL2_FIXED_OCCASION_COVERAGE_AUDIT.sql` استفاده شود.
- این نسخه DDL جدید ندارد.

## تغییرات نسخه 0.3.76

- Breadcrumb سراسری به `AppShell` اضافه شده و در تمام صفحات سامانه نمایش داده می‌شود.
- هر سطح قابل بازگشت لینک دارد؛ برای نمونه: `اطلاعات پایه ← تقویم دو ← روزهای تقویم کاری`.
- عنوان صفحه جاری از H1 همان صفحه دریافت می‌شود تا فرم‌های Dynamic نیز عنوان فارسی صحیح داشته باشند.
- Breadcrumbهای محلی قبلی مخفی شده‌اند تا مسیر ناوبری دوباره نمایش داده نشود.

## تغییرات نسخه 0.3.75

- FIX83: Renderer اختصاصی `CAL2.BUSINESS_CALENDAR_DAY` بر اساس `tableName` نیز فعال می‌شود تا صفحه به Grid عمومی سقوط نکند.
- Grid روزهای تقویم کاری تاریخ شمسی، روز هفته، نام فارسی تقویم/منبع و برچسب فارسی وضعیت و دلیل را نمایش می‌دهد.
- پس از Build، برای بارگذاری Lazy Chunk جدید مرورگر یک Hard Refresh انجام شود.

## تغییرات نسخه 0.3.73

- فرم «تبدیل نام فارسی به انگلیسی» Candidateهای نیازمند Review را نیز نمایش می‌دهد.
- Resolver کامل Oracle علاوه بر English، روش تبدیل، Confidence و وضعیت Auto-fill را به UI برمی‌گرداند.
- برای نمونه `آقا مصطفی` و `آقامصطفی` می‌توانند `Agha Mostafa` را به‌عنوان پیشنهاد نمایش دهند، بدون اینکه Auto-fill مجاز شود.

## تغییرات نسخه 0.3.72

- ابزار «تبدیل نام فارسی به انگلیسی» در اطلاعات پایه عمومی اضافه شده است.
- Backend از `POST /api/v1/name-romanization/resolve` استفاده می‌کند و مستقیماً `GEO.FN_ROMANIZE_NAME` را در Oracle فراخوانی می‌کند.
- نتیجه NULL به‌عنوان «نیازمند بازبینی حاکمیتی / غیرمجاز برای Auto-fill» نمایش داده می‌شود و با Transliteration حدسی جایگزین نمی‌شود.
...
از نسخه `0.3.70` صفحه `/fee` بر مبنای مدل Oracle و Seed Data Baseline 1.0 پیوست بازطراحی شده است. مدل جاری 47 جدول دارد و Seed پایه 574 رکورد را پوشش می‌دهد. صفحه اصلی کارمزد دارای یک فلوچارت بصری و تعاملی است که ترتیب پیشنهادی تعریف اطلاعات را با Shape و Connector نمایش می‌دهد و هر Shape را مستقیماً به فرم مربوط متصل می‌کند. فرم `FEE_CALCULATION_RULE` نیز با عنوان «نوع و قاعده محاسبه کارمزد» و انتخاب سریع مبلغ ثابت/درصدی/پلکانی/ترکیبی ارائه می‌شود. از FIX81، تعرفه‌های کامل بانک مرکزی ۱۴۰۴ نیز از فایل Excel پیوست با Import مستقل تمیز می‌شوند: 239 ردیف منبع به 229 تعریف منطقی، 15 Tier ارزیابی و Ruleهای امن/External تبدیل می‌شوند. در FIX98، مصوبه جدید کارمزد خدمات بانکی ریالی به‌صورت موقت و Versioned وارد می‌شود: 152 تعرفه، 180 جزء محاسباتی و 15 Tier ارزیابی ثبت می‌شوند؛ 156 نسخه قبلی غیرالکترونیکی با Temporal Archive بسته می‌شوند و 73 تعرفه الکترونیکی قبلی حفظ می‌شوند. چون نامه ابلاغ رسمی در اختیار پروژه نیست، Source جدید با وضعیت `PROVISIONAL` ثبت می‌شود. از FIX101، اسکریپت Read-only تطبیق Source-to-Oracle نیز اضافه شده است که علاوه بر 152 Rule، تمام 180 Component، 66 Input، 15 Tier، Hash منبع و `CONFIG_HASH` هر تعرفه را کنترل می‌کند.
...
- Import تعرفه بانک مرکزی ۱۴۰۴: `database/oracle/fee/install-cbi-fee-1404.sql`
