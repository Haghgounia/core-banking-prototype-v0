...
- Grid «روزهای تقویم کاری» بازطراحی شد و تاریخ هجری شمسی، روز هفته، عنوان فارسی تقویم و عنوان فارسی منبع را نمایش می‌دهد.
- Codeهای فنی `UNCLASSIFIED`، `PUBLIC_HOLIDAY` و `PENDING_RULE_EVALUATION` در Grid با عنوان فارسی نمایش داده می‌شوند؛ مقدار ذخیره‌شده در Oracle تغییر نمی‌کند.
- جستجوی این صفحه اکنون تاریخ شمسی، عنوان/کد تقویم، روز هفته، وضعیت، دلیل و منبع را پوشش می‌دهد.
