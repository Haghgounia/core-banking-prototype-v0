package com.behsazan.corebanking.calendar.reference.domain;
...
public final class CalendarReferenceModels {
    private CalendarReferenceModels() {}

    public enum FieldType { TEXT, NUMBER, DATE, TIME, BOOLEAN, SELECT, LOOKUP }
    ...
    public record TableDescriptor(
        ...
        public List<FieldDescriptor> keyFields() {
            return fields.stream().filter(FieldDescriptor::key).toList();
        }

        public List<FieldDescriptor> gridFields() {
            return fields.stream().filter(FieldDescriptor::grid).toList();
        }

        public List<FieldDescriptor> searchableFields() {
            return fields.stream().filter(FieldDescriptor::searchable).toList();
        }

        public FieldDescriptor field(String apiName) {
            return fields.stream().filter(field -> field.apiName().equals(apiName)).findFirst()
                    .orElseThrow(() -> new IllegalArgumentException("Unknown calendar field: " + apiName));
        }
    }
