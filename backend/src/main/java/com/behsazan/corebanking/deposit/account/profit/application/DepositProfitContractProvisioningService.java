...
public class DepositProfitContractProvisioningService {
    ...
    public void ensureForActivation(long accountId,long openingRequestId,long productVersionId,String actor){
        ...
        if(repository.profile(accountId).isEmpty()){
            int created=repository.provisionProfileFromOpening(accountId,openingRequestId,actor);
            if(created!=1)throw new DepositAccountLifecycleException(
                    "Profit Profile عملیاتی از Snapshot افتتاح ایجاد نشد.",
                    Map.of("DEPOSIT_ACCOUNT_PROFIT_PROFILE.ACCOUNT_ID",String.valueOf(accountId),"DEPOSIT_OPENING_PROFIT_INSTRUCTION.OPENING_REQUEST_ID",String.valueOf(openingRequestId)));
        }
        if(repository.contract(accountId).isEmpty()){
