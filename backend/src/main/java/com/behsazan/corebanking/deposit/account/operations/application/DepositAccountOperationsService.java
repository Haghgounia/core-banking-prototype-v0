...
public class DepositAccountOperationsService {
    ...
    public DepositAccountOperationsService(DepositAccountOperationsRepository repository){this.repository=repository;}
    ...
    @Transactional(readOnly=true) public AccountDetails get(long accountId){if(accountId<=0)throw new IllegalArgumentException("accountId باید مثبت باشد.");AccountSummary a=repository.find(accountId).orElseThrow(()->new DepositAccountNotFoundException("حساب سپرده با شناسه "+accountId+" یافت نشد."));return new AccountDetails(a,repository.parties(accountId),repository.contacts(accountId),repository.servicingHistory(accountId),repository.events(accountId),repository.statusHistory(accountId),repository.holds(accountId),repository.holdHistory(accountId));}
